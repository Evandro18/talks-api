...
const talksList = populatedTrails.reduce((acc, trails, i) => {
  ...
  const data = [...calculateStartTime(config.morning.start, config.morning.end, morning.talks), ...calculateStartTime(config.afternoon.start, config.afternoon.end, afternoon.talks)]
